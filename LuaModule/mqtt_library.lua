...
--print(message_type)
  ...
  local msg = string.char(MQTT.Utility.shift_left(message_type, 4))
  if(message_type == MQTT.message.TYPE_SUBSCRIBE) then
      msg = string.char(130)
  end
  --local mes = mes..string.char(MQTT.Utility.shift_left(MQTT.message.TYPE_CONACK, 4))
  
  --print(#mes)

  if (payload == nil) then
    message = message .. string.char(0)  -- Zero length, no payload
  --  mes = mes.. string.char(0)
  else
...
--print(remaining_length)
  ...
  local status, error_message = self.socket_client:send(msg)
