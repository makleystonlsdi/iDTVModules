...
function publishDicoverySmartObject()
  ...
  lua_module.mqtt_lua_module.post('/smart_object_discovery/'..obj.environment..'/'..obj.controllable..'/'..obj.type, objJ)
  if(log)then
...
function publishDisconnectSmartObject()
  if(getSizeSmartObjectTable()==0)then
    ...
    if(smartObjectsPresent[index])then
      ...
      lua_module.mqtt_lua_module.post('/smart_object_disconnected', obj.id)
      if(log)then
...
function publishReadSmartObject()
  if(getSizeSmartObjectTable()==0)then
    ...
    if(smartObjectsPresent[index]) then
      ...
      lua_module.mqtt_lua_module.post('/smart_object_read/'..smartObjectsPresent[index].environment..'/'..smartObjectsPresent[index].controllable..'/'..smartObjectsPresent[index].type, json.encode(smartObjectsPresent[index]))
      if(log)then
...
function publishReadPortableDevice()
  ...
  for k, v in pairs(devicesPresents)do
    ...
    lua_module.mqtt_lua_module.post('/portable_device/'..v.environment..'/'..v.type, vJ)
    if(log)then
...
function publishIsAlivePortableDevice()
  ...
  for k, v in pairs(devicesPresents)do
    lua_module.mqtt_lua_module.post('/alive_portable_device', v.id)
    if(log)then
